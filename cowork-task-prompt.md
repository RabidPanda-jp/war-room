...
Also look up this week's TV/streaming network for every game involving a rostered player in either league (starters and bench) from **https://www.nfl.com/schedules/** — both teams in a game get the same network. Use short, recognizable names: `CBS`, `FOX`, `NBC`, `ESPN`, `ABC`, `NFL Network`, `Prime`, `Peacock`, `Netflix`. If nfl.com still shows the slot as TBD (common for late-season Sunday afternoon flex games), omit that team rather than guessing. Emit this as the top-level `broadcasts` map (schema below) — the app shows it next to the kickoff time on the Team screen.
...
- **TV/streaming network comes from nfl.com/schedules, not ESPN's API.** ESPN's scoreboard JSON exposes broadcast info under undocumented, inconsistent fields that the app can't reliably parse client-side every load. Instead, read the announced network per game off `https://www.nfl.com/schedules/` each run and write it into the `broadcasts` map. Never guess a network that page doesn't show — omit that team instead.
